				    ...
				    u16 size, void *data)
{
	...
	if((status = usb_submit_urb(urb, GFP_ATOMIC)) < 0) {
		axerr(dev, "Error submitting the control message: status=%d",
				status);
		kfree(req);
		usb_free_urb(urb);
	}
...
static int ax8817x_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	if ((ret = ax8817x_write_cmd(dev, AX_CMD_WRITE_RX_CTL,
				0x80, 0, 0, buf)) < 0) {
		axerr(dev, "send AX_CMD_WRITE_RX_CTL failed: %d", ret);
		goto out2;
	}
	...
	if ((ret = ax8817x_read_cmd(dev, AX_CMD_READ_NODE_ID,
				0, 0, 6, buf)) < 0) {
		axerr(dev, "read AX_CMD_READ_NODE_ID failed: %d", ret);
		goto out2;
	}
	...
	if ((ret = ax8817x_read_cmd(dev, AX_CMD_READ_PHY_ID,
				0, 0, 2, buf)) < 0) {
		axerr(dev, "error on read AX_CMD_READ_PHY_ID: %02x", ret);
		goto out2;
	} else if (ret < 2) {
		/* this should always return 2 bytes */
		axerr(dev, "Read PHYID returned less than 2 bytes: ret=%02x",
				ret);
		ret = -EIO;
		goto out2;
	}
...
static int ax88772_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	if (dev->mii.phy_id == 0x10)
	{
		if ((ret = ax8817x_mdio_read_le(dev->net,
			...
			axerr(dev, "Read PHY register 2 must be 0x3b00: %d",
				ret);
			goto out2;
		}
...
static int ax88772a_phy_powerup (struct usbnet *dev)
{
	...
	if ((ret = ax8817x_write_cmd(dev, AX_CMD_SW_RESET,
		...
		axerr(dev, "Failed to reset PHY: %d", ret);
		return ret;
	}
	...
	if ((ret = ax8817x_write_cmd(dev, AX_CMD_SW_RESET,
		...
		axerr(dev, "Failed to power up PHY: %d", ret);
		return ret;
	}
...
static int ax88772a_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	if (!ax772a_data) {
		axerr(dev, "Cannot allocate memory for AX88772A data");
		kfree (buf);
		return -ENOMEM;
	}
	...
	if ((ret = ax8817x_read_cmd(dev, AX_CMD_READ_EEPROM, 
		...
		axerr(dev, "read SROM address 17h failed: %d", ret);
		goto out2;
	}
	...
	if(dev->mii.phy_id != 0x10) {
		axerr(dev, "Got wrong PHY ID: %02x", dev->mii.phy_id);
		goto out2;
	}
...
static int ax88772b_set_csums(struct usbnet *dev)
{
	...
	if (ax772b_data->checksum & AX_TX_CHECKSUM) {
		checksum = (AX_TXCOE_TCP | AX_TXCOE_UDP | 
...
static int ax88772b_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	if (!ax772b_data) {
		axerr(dev, "Cannot allocate memory for AX88772B data");
		kfree (buf);
		return -ENOMEM;
	}
	...
	if ((ret = ax8817x_write_cmd(dev, AX_CMD_WRITE_GPIOS,
			AXGPIOS_RSE, 0, 0, buf)) < 0) {
		axerr(dev, "Failed to enable GPIO finction: %d", ret);
		goto err_out;
	}
	msleep(5);
	...
	if ((ret = ax8817x_read_cmd (dev, AX_CMD_READ_EEPROM,
		...
		axerr(dev, "read SROM address 18h failed: %d", ret);
		goto err_out;
	}
	...
	for (i = 0; i < (ETH_ALEN >> 1); i++) {
		if ((ret = ax8817x_read_cmd (dev, AX_CMD_READ_EEPROM,
			...
			axerr(dev, "read SROM address 04h failed: %d", ret);
			goto err_out;
		}
	}
	memcpy(dev->net->dev_addr, buf, ETH_ALEN);
	...
	if ((ret = ax8817x_write_cmd (dev, AX88772_CMD_WRITE_NODE_ID,
		...
		axerr(dev, "set MAC address failed: %d", ret);
		goto err_out;
	}
	...
	if ((ret = ax8817x_write_cmd(dev, AX_CMD_WRITE_MEDIUM_MODE,
				AX88772_MEDIUM_DEFAULT, 0, 0, buf)) < 0) {
		axerr(dev, "Failed to write medium mode: %d", ret);
		goto err_out;
	}
	...
	if ((ret = ax8817x_write_cmd(dev, AX_CMD_WRITE_IPG0,
			AX88772A_IPG0_DEFAULT | AX88772A_IPG1_DEFAULT << 8,
			AX88772A_IPG2_DEFAULT, 0, buf)) < 0) {
		axerr(dev, "Failed to write interframe gap: %d", ret);
		goto err_out;
	}
	...
	if ((ret = ax88772b_set_csums(dev)) < 0) {
		axerr(dev, "Write RX_COE/TX_COE failed: %d", ret);
		goto err_out;
	}

	if (dev->udev->speed == USB_SPEED_HIGH) {
		if ((ret = ax8817x_write_cmd (dev, 0x2A,
				0x8400, 0x851E, 0, buf)) < 0) {
			axerr(dev, "Reset RX_CTL failed: %d", ret);
			goto err_out;
		}

		dev->rx_urb_size = AX88772B_MAX_BULKIN_LEN;

	} else {
		if ((ret = ax8817x_write_cmd (dev, 0x2A,
				0x8000, 0x8001, 0, buf)) < 0) {
			axerr(dev, "Reset RX_CTL failed: %d", ret);
			goto err_out;
		}
		dev->rx_urb_size = 2048;
	...
	if ((ret = ax8817x_write_cmd (dev, AX_CMD_SW_RESET,
		...
		axerr(dev, "Failed to configure PHY power saving: %d", ret);
		goto err_out;
	}
...
static int ax88178_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	if (!ax178dataptr) {
		axerr(dev, "Cannot allocate memory for AX88178 data");
		ret = -ENOMEM;
		goto error_out;
	}
	...
	if (ax178dataptr->UseGpio0) {
		...
		if (ax178dataptr->PhyMode == PHY_MODE_MARVELL) {
			...
			if ((ret = ax8817x_write_cmd (dev, AX_CMD_WRITE_GPIOS,
					...
					0, 0, NULL)) < 0) {
				axerr(dev, "write GPIO failed: %d", ret);
				goto error_out;
			}
	...
	if ((ret = ax8817x_write_cmd(dev,
			AX_CMD_SW_PHY_SELECT, 0, 0, 0, buf)) < 0) {
		axerr(dev, "Select PHY failed: %d", ret);
		goto error_out;
	}

	if ((ret = ax8817x_write_cmd(dev, AX_CMD_SW_RESET,
			AX_SWRESET_IPPD | AX_SWRESET_PRL, 0, 0, buf)) < 0) {
		axerr(dev, "Issue sw reset failed: %d", ret);
		goto error_out;
	}

	if ((ret = ax8817x_write_cmd(dev, AX_CMD_WRITE_RX_CTL,
			0, 0, 0, buf)) < 0) {
		axerr(dev, "Issue rx ctrl failed: %d", ret);
		goto error_out;
	}
	...
	if ((ret = ax8817x_write_cmd(dev, AX_CMD_WRITE_RX_CTL,
			...
			0, 0, NULL)) < 0) {
		axerr(dev, "write RX ctrl reg failed: %d", ret);
		goto error_out;
	}
...
static int ax88772b_rx_fixup(struct usbnet *dev, struct sk_buff *skb)
{
	...
	while (skb->len > 0) {
		...
		if (rx_hdr.len > (ETH_FRAME_LEN + 4)) {
			axerr(dev, "invalid rx length %d", rx_hdr.len);
			return 0;
		}
...
ax88772b_tx_fixup(struct usbnet *dev, struct sk_buff *skb, unsigned flags)
{
	struct ax8817x_data *data = (struct ax8817x_data *)&dev->data;
	struct ax88772b_data *ax772b_data = data->priv.ax772b_data;
	int padlen;
...
static int ax88178_set_media(struct usbnet *dev)
{
	...
	if ((ret = ax8817x_write_cmd(dev, AX_CMD_WRITE_MEDIUM_MODE, 
			media, 0, 0, NULL)) < 0) {
		axerr(dev, "write mode medium reg failed: %d", ret);
		return ret;
	}
