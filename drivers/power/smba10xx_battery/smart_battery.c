		...
		enum power_supply_property psp, union power_supply_propval *val)
{
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_CAPACITY:
			...
			val->intval = (battery_dev.rsoc<8) ? 0 : (battery_dev.rsoc-3)*100/97;
			#else
...
int update_battery_state()
{
	...
	unsigned short voltage_now, voltage_avg, remaincapacity, designcapacity;
...
//		|| StateOfHealth(&battery_dev.dev, &health))
		|| RemainingCapacity(&battery_dev.dev, &remaincapacity)
		|| DesignCapacity(&battery_dev.dev, &designcapacity))
	{
	...
	// fix by xiaolu	
	//if (rsoc == 100 ) {
		//battery_dev.rsoc = voltage_now / 84;
		//battery_dev.rsoc = (voltage_now - 6800) / 16;
	//} else {
...
//	printk("RelativeStateOfChage=%d\r\n", battery_dev.rsoc);
	logd("RelativeStateOfChage=%d\r\n", rsoc);
	logd("Voltage=%d\r\n", battery_dev.voltage_now);
	logd("Remaincapacity=%d\r\n", remaincapacity);
	logd("Designcapacity=%d\r\n", designcapacity);
